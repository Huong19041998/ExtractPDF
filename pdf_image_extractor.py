...
class PDFImageExtractor:
    def __init__(
        ...
        self.caption_ratio = (
            0.25  # Ratio for determining the caption area below images
        )
    ...
    def is_pdf_scan_based_on_metadata(self, metadata) -> Optional[bool]:
        ...
        if any(scanner in creator for scanner in scanners) or any(
            scanner in producer for scanner in scanners
        ):
            return True  # PDF is likely scanned
        if any(tool in creator for tool in office_tools) or any(
            tool in producer for tool in office_tools
        ):
    ...
    def is_pdf_scan(self) -> bool:
        ...
        # Check with OCR
        return (
            self.is_pdf_scan_with_ocr()
        )  # Return True if OCR indicates scan, else False
    ...
    def extract_images_from_pymupdf(self):
        ...
        for page_num in range(len(doc)):  # Iterate over each page in the PDF
            ...
            pix = page.get_pixmap(
                dpi=self.dpi
            )  # Render the page as an image with a DPI of self.dpi
            img_page = np.frombuffer(pix.samples, dtype=np.uint8).reshape(
                pix.height, pix.width, pix.n
            )  # Convert the pixmap to a numpy array
            img_page = cv2.cvtColor(
                img_page, cv2.COLOR_RGB2BGR
            )  # Convert the image from RGB to BGR color space
            cv2.imwrite(f"{self.output_dir}/page_{page_num + 1}.png", img_page)
            if self.use_pymupdf_for_caption:
                captions_page, caption_boxes = (
                    self.extract_captions_with_pymupdf(page)
                )
            for img_index, img in enumerate(
                ...
                rect = page.get_image_rects(img)[
                    0
                ]  # Get the rectangle coordinates of the image
                x0, y0, x1, y1 = (
                    int(rect.x0 * self.scale_factor),
                    int(rect.y0 * self.scale_factor),
                    int(rect.x1 * self.scale_factor),
                    int(rect.y1 * self.scale_factor),
                ...
                y2_caption = min(
                    y2_caption, img_page.shape[0]
                )  # Ensure the caption height doesn't exceed the image height

                # Adjust caption boundaries to include some padding
                x0_caption = 0 if 0 <= x0 < 10 else x0 - 10
                x1_caption = (
                    x1
                    if img_page.shape[1] - 10 <= x1 <= img_page.shape[1]
                    else min(x1 + 10, img_page.shape[1])
                )

                # Crop the caption area from the image
                cropped_cap_image = img_page[
                    y1:y2_caption, x0_caption:x1_caption
                ]

                # Save the cropped image and caption image
                image_filename = os.path.join(
                    self.output_dir,
                    f"page_{page_num + 1}_cropped_image_{img_index + 1}.png",
                )
                cv2.imwrite(image_filename, cropped_image)
                image_filename_cap = os.path.join(
                    self.output_dir,
                ...
                if self.use_pymupdf_for_caption:
                    ...
                    for index_cap, caption_box in enumerate(caption_boxes):
                        if self.is_text_caption_for_image(
                ...
                self.images_dict[f"page_{page_num + 1}_{img_index + 1}"] = (
                    cropped_image  # Store the cropped image
                )
                self.captions_dict[f"page_{page_num + 1}_{img_index + 1}"] = (
    ...
    def extract_images_from_opencv(self, index_page, image_page):
        ...
        minimum_width = (
            100  # Minimum width for the bounding rectangle of detected images
        )
        minimum_height = (
            100  # Minimum height for the bounding rectangle of detected images
        )
        # image = cv2.imread(image_page)
        # original = image_page.copy()
        gray = cv2.cvtColor(
            image_page, cv2.COLOR_BGR2GRAY
        ...
        for c in cnts:  # Iterate over each contour found
            ...
            x1_caption = (
                x1
                if image_page.shape[1] - 10 <= x1 <= image_page.shape[1]
                else min(x1 + 10, image_page.shape[1])
            )
            ...
            if w >= minimum_width and h >= minimum_height:
                ...
                cropped_cap_image = image_page[
                    y1:y2_caption, x0_caption:x1_caption
                ]
                out_image_cap = os.path.join(
                    self.output_dir,
                    f"page_{index_page + 1}_cap_{cropped_cap_image_number}.png",
                )
                cv2.imwrite(
                    out_image_cap, cropped_cap_image
                )  # Save the cropped caption image
                out_image = os.path.join(
                    self.output_dir,
                    f"page_{index_page + 1}_{cropped_cap_image_number}.png",
                )
                cv2.imwrite(
                ...
                caption = self.extract_caption_from_image(cropped_cap_image)
                self.images_dict[
                    f"page_{index_page + 1}_{cropped_cap_image_number}"
                ] = cropped_image  # Store the cropped caption image
                self.captions_dict[
                    f"page_{index_page + 1}_{cropped_cap_image_number}"
                ] = caption  # Store the caption
                cropped_cap_image_number += (
                    1  # Increment the cropped caption image counter
                )

        # If tracing is enabled, save a trace image of the original input image
        if self.tracing_enabled:
            trace_image = os.path.join(
                self.tracing_dir, f"page_{index_page + 1}_trace.png"
            )
            cv2.imwrite(trace_image, image_page)  # Save the trace image

    def extract_captions_with_pymupdf(
        self, page: fitz.Page
            ...
            if block["type"] == 0:  # Type 0 means it's a text block
                for line in block["lines"]:
                    for span in line["spans"]:
                        ...
                        bbox = span["bbox"]
                        bbox_scaled = [
                            int(coord * self.scale_factor) for coord in bbox
                        ]
                        captions_page.append(text)
                        box_captions.append(bbox_scaled)
    ...
    def is_text_caption_for_image(self, image_box, text_box) -> bool:
        ...
        if (
            y1_img
            <= y0_text
            <= y1_img + int((y1_img - y0_img) * self.caption_ratio)
            and x0_img - 100 * self.scale_factor
            <= x0_text
            <= x0_img + 100 * self.scale_factor
        ):
            ...
            if (y1_text - y0_text) <= self.caption_ratio * image_height:
                return True
    ...
    def convert_pdf_to_images(self):
        ...
        for i in range(len(doc)):
            ...
            pix = page.get_pixmap(dpi=self.dpi)  # Get a pixmap of the page
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract images and captions from a PDF."
    )
    ...
    parser.add_argument(
        "--tracing_enabled", action="store_true", help="Enable tracing mode."
    )
    parser.add_argument(
        "--tracing_dir",
        type=str,
        default="tracing",
        help="Directory to save tracing images.",
    )
    parser.add_argument(
        "--use_pymupdf_for_caption",
        type=bool,
        default=True,
